...
if (mascota) mascota.style.display = (s === 'welcome' && personajeActual !== 'ninguno') ? 'block' : 'none';
  ['welcome','teoria','preguntas','autoevaluacion','consulta','multiple','micontenido','calendario','superadmin','settings','plan','repaso'].forEach(id => {
  ...
  const titles = { welcome: 'Inicio', teoria: 'Teoría', preguntas: 'Preguntas de Final', autoevaluacion: 'Autoevaluación', micontenido: 'Mi Contenido', plan: 'Plan de Estudio', repaso: 'Repaso' };
...
if (s === 'repaso') renderRepaso();
if (s === 'plan') renderPlan();
if (s === 'welcome') {
  setTimeout(() => verificarRepasosHoy(), 800);
  setTimeout(() => renderWidgetPlanHoy(), 800);
}
...
function handleSearch(val) {
  ...
  if (!searchQuery) {
    ...
    buscarEnTodasLasUnidades(searchQuery, count);
  }
}
...
async function buscarEnTodasLasUnidades(query, countEl) {
  const grid = document.getElementById('units-grid');
  grid.innerHTML = '<p style="color:var(--muted2);padding:24px">Buscando...</p>';

  const visibleIds = await getVisibleUserIds();
  const unidadesFiltradas = currentMateria
    ? mcUnidades.filter(u => visibleIds.includes(u.alumno_id) && u.grupo_id === currentMateria)
    : mcUnidades.filter(u => visibleIds.includes(u.alumno_id));

  const unidadIds = unidadesFiltradas.map(u => u.id);
  if (!unidadIds.length) {
    grid.innerHTML = '<p style="color:var(--muted2);padding:24px">Sin resultados.</p>';
  ...
  const q = query.toLowerCase();
  const { data: filminas } = await supabaseClient
    .from('filminas').select('*, unidades(numero, nombre)')
    .in('unidad_id', unidadIds)
    .or(`titulo.ilike.%${q}%,contenido.ilike.%${q}%`);

  const found = filminas || [];
  if (countEl) countEl.textContent = `${found.length} resultado${found.length !== 1 ? 's' : ''}`;
  ...
  found.forEach(f => {
    ...
    const nombre = f.unidades?.nombre;
    if (!porUnidad.has(num)) porUnidad.set(num, { numero: num, nombre, count: 0 });
    porUnidad.get(num).count++;
  });

  const unidadesOrdenadas = [...porUnidad.values()].sort((a, b) => a.numero - b.numero);
  grid.innerHTML = unidadesOrdenadas.map(u => `
    <div class="unit-card" onclick="openUnidad(${u.numero})">
      <div class="uc-num">${u.numero}</div>
      <div class="uc-body">
        <div class="uc-title">${u.nombre}</div>
        <div class="uc-sub">${u.count} filmina${u.count !== 1 ? 's' : ''} con resultados</div>
      </div>
...
function selectMateria() {
  ...
  // ✨ Refrescar widget del plan al cambiar de materia
  setTimeout(() => renderWidgetPlanHoy(), 300);
}
...
const grupoIdCapturado = selectedOption?.dataset?.grupo || currentPerfil?.grupo_id;
  ...
  alert(`El PDF tiene ${pdf.numPages} páginas. Máximo 40 páginas.`);
    ...
    // ✨ Dividir texto en bloques más pequeños (4000 chars) para reducir timeouts
    ...
    let todasFilminas = [];
    const bloquesFallidos = []; // ✨ Tracking de bloques que fallaron incluso después del reintento

    // ✨ Función helper para procesar un chunk con reintentos
    async function procesarChunk(chunk, intento = 1) {
...
${chunk}` }]
        ...
        console.warn(`Intento ${intento}/${MAX_INTENTOS} falló:`, e.message);
        if (intento < MAX_INTENTOS) {
          // Espera incremental: 5s, 10s, 15s
          await new Promise(r => setTimeout(r, 5000 * intento));
          return procesarChunk(chunk, intento + 1);
        }
    ...
    for (let ci = 0; ci < chunks.length; ci++) {
      ...
      try {
        const bloque = await procesarChunk(chunks[ci]);
        todasFilminas = todasFilminas.concat(bloque);
      } catch (e) {
        console.error(`❌ Bloque ${ci + 1} falló definitivamente:`, e);
        bloquesFallidos.push(ci + 1);
      }
    ...
    // ✨ Aviso visible si hubo bloques fallidos
    const aviso = bloquesFallidos.length > 0
      ? `<div style="background:rgba(245,158,11,0.1); border:1px solid rgba(245,158,11,0.3); border-radius:8px; padding:10px 14px; margin-bottom:12px; font-size:13px; color:#f59e0b;">
          ⚠️ Atención: ${bloquesFallidos.length} bloque${bloquesFallidos.length !== 1 ? 's' : ''} (${bloquesFallidos.join(', ')}) no se pudo procesar. Algunas filminas pueden faltar. Te recomendamos subir el PDF de nuevo más tarde para completarlas.
        </div>`
      : '';
...
async function verificarRepasosHoy() {
  ...
  try {
    ...
    // Traer filminas (filtradas por materia si corresponde)
    ...
    // Contar pendientes (riesgo + por mejorar)
    let enRiesgo = 0;
    let porMejorar = 0;

    filminas.forEach(f => {
      const fScores = scoresPorFilmina[parseInt(f.id)];
      if (!fScores?.length) return;
      const ultimo = fScores[0].puntaje;
      const anteultimo = fScores[1]?.puntaje;
      if (anteultimo !== undefined && ultimo >= 7 && anteultimo >= 7) return;
      if (ultimo < 5) enRiesgo++;
      else porMejorar++;
    });

    const total = enRiesgo + porMejorar;
    ...
    let mensaje = '';
    if (enRiesgo > 0) {
      mensaje = `⚠️ Tenés <strong>${enRiesgo}</strong> filmina${enRiesgo !== 1 ? 's' : ''} en riesgo y <strong>${porMejorar}</strong> por mejorar. <a href="#" onclick="showSection('repaso');return false;">Ir a repasar →</a>`;
    } else {
      mensaje = `📈 Tenés <strong>${porMejorar}</strong> filmina${porMejorar !== 1 ? 's' : ''} para repasar. <a href="#" onclick="showSection('repaso');return false;">Ir a repasar →</a>`;
    }

    banner.innerHTML = mensaje;

  } catch (e) {
    console.error('Error verificarRepasosHoy:', e);
    banner.style.display = 'none';
  }
}
...
function iniciarPomodoro() {
  ...
  document.querySelectorAll('[onclick*="iniciarVF"]').forEach(el => el.style.display = 'none');
  document.querySelectorAll('[onclick*="openAutoevaluacion"]').forEach(el => el.style.display = 'none');
  document.querySelectorAll('[onclick*="abrirFlashcards"]').forEach(el => el.style.display = 'none');
  const pBtn = document.getElementById('pomodoro-btn');
  if (pBtn) pBtn.style.display = 'none';
  const pRing = document.getElementById('pomodoro-ring');
  if (pRing) pRing.style.display = 'inline-flex';
  const pBtnRepaso = document.getElementById('pomodoro-btn-repaso');
  if (pBtnRepaso) pBtnRepaso.style.display = 'none';
  const pRingRepaso = document.getElementById('pomodoro-ring-repaso');
  if (pRingRepaso) pRingRepaso.style.display = 'inline-flex';
...
function actualizarRing() {
  ...
  document.querySelectorAll('.ring-progress').forEach(el =>
    el.setAttribute('stroke-dashoffset', circunferencia * (1 - ratio))
  );
  const m = String(Math.floor(pomodoroSegundos / 60)).padStart(2, '0');
  const s = String(pomodoroSegundos % 60).padStart(2, '0');
  document.querySelectorAll('.ring-time').forEach(el => el.textContent = `${m}:${s}`);
}
...
function finalizarPomodoro() {
  ...
  document.querySelectorAll('[onclick*="iniciarVF"]').forEach(el => el.style.display = '');
  document.querySelectorAll('[onclick*="openAutoevaluacion"]').forEach(el => el.style.display = '');
  document.querySelectorAll('[onclick*="abrirFlashcards"]').forEach(el => el.style.display = '');
  const pBtn = document.getElementById('pomodoro-btn');
  if (pBtn) pBtn.style.display = '';
  const pRing = document.getElementById('pomodoro-ring');
  if (pRing) pRing.style.display = 'none';
  const pBtnRepaso = document.getElementById('pomodoro-btn-repaso');
  if (pBtnRepaso) pBtnRepaso.style.display = '';
  const pRingRepaso = document.getElementById('pomodoro-ring-repaso');
  if (pRingRepaso) pRingRepaso.style.display = 'none';
...
async function enviarEvaluacionFilminas() {
  ...
  const evaluaciones = await Promise.all(resumenes.map(async ({ filmina, texto }) => {
    ...
    const prompt = `Sos un evaluador de aprendizaje universitario de contabilidad. Tu única tarea es comparar lo que escribió el alumno contra el contenido exacto de la filmina.
...
async function renderPlanContent() {
  ...
  try {
    ...
    // ✨ Estudiadas hoy = scores de hoy con puntaje ≥ 5 (las mal aprobadas no cuentan)
    const hoyStr = new Date().toISOString().split('T')[0];
    const estudiadasHoyIds = new Set(
      scores
        .filter(s => (s.created_at || '').startsWith(hoyStr) && s.puntaje >= 5)
...
function showPlanTab(tab) {
  const wraps = { plan: 'plan-wrap', evolucion: 'evolucion-wrap', filminas: 'filminas-estado-wrap', pomodoro: 'pomodoro-resumen-wrap' };
  ['plan', 'evolucion', 'filminas', 'pomodoro'].forEach(t => {
  ...
  else if (tab === 'pomodoro') renderResumenPomodoro();
}

async function renderResumenPomodoro() {
  const wrap = document.getElementById('pomodoro-resumen-wrap');
  wrap.innerHTML = `<div style="text-align:center;padding:40px 0;color:var(--muted)"><div style="font-size:32px;margin-bottom:12px">⏳</div><p>Cargando historial...</p></div>`;

  try {
    const visibleIds = await getVisibleUserIds();

    const { data: todasSesionesPomodoro } = await supabaseClient
    ...
    let sp = todasSesionesPomodoro || [];
    if (currentMateria) {
      const { data: unidades } = await supabaseClient.from('unidades').select('id').in('alumno_id', visibleIds).eq('grupo_id', currentMateria);
      const { data: filminas } = await supabaseClient.from('filminas').select('id, unidad_id').in('alumno_id', visibleIds);
      const unidadIds = new Set((unidades || []).map(u => u.id));
      const filminaIds = new Set((filminas || []).filter(f => unidadIds.has(f.unidad_id)).map(f => f.id));
      sp = sp.filter(s => (s.filminas_ids || []).some(id => filminaIds.has(id)));
    }
    ...
    if (!sp.length) {
      wrap.innerHTML = `
        <div style="text-align:center;padding:40px 20px;color:var(--muted2);">
          ...
          <p style="font-size:12px;margin-top:4px;">Completá una sesión para ver tu historial acá.</p>
        </div>`;
      return;
    }

    const conPuntaje = sp.filter(s => s.puntaje != null);
    const promPuntaje = conPuntaje.length > 0
      ? Math.round(conPuntaje.reduce((a, b) => a + b.puntaje, 0) / conPuntaje.length)
      : 0;
    ...
    wrap.innerHTML = `
      ...
      <div id="pomodoro-sesiones-lista" style="display:flex; flex-direction:column; gap:10px;" data-sesiones='${JSON.stringify(sp.map(s => ({...s, resumen: s.resumen, feedback_ia: s.feedback_ia}))).replace(/'/g, "&#39;")}'>
    ...
    console.error('Error renderResumenPomodoro:', e);
    wrap.innerHTML = '<p style="color:var(--muted2);padding:20px;">Error al cargar el historial.</p>';
  }
}
...
async function renderRepaso() {
  const wrap = document.getElementById('repaso-wrap');
  wrap.innerHTML = `<div style="text-align:center;padding:40px 0;color:var(--muted)"><div style="font-size:32px;margin-bottom:12px">⏳</div><p>Analizando tu progreso...</p></div>`;

  try {
    const visibleIds = await getVisibleUserIds();

    // Traer filminas (incluye orden y unidad para sugerencias)
    let filminasQuery = supabaseClient
      .from('filminas')
      .select('id, titulo, orden, unidad_id, unidades(numero, nombre)')
      .in('alumno_id', visibleIds);
    if (currentMateria) filminasQuery = filminasQuery.eq('grupo_id', currentMateria);
    const { data: todasFilminas } = await filminasQuery;

    // Traer scores (más reciente primero)
    const { data: allScores } = await supabaseClient
      .from('filmina_scores')
      .select('filmina_id, puntaje, created_at')
      .eq('alumno_id', currentUser.id)
    ...
    // Clasificar
    const grupos = { riesgo: [], mejorar: [] };
    filminas.forEach(f => {
      const fScores = scoresPorFilmina[parseInt(f.id)];
      if (!fScores?.length) return;
      const ultimo = fScores[0].puntaje;
      const anteultimo = fScores[1]?.puntaje;
      // Las aprendidas (≥7 dos veces) se omiten en esta pantalla
      if (anteultimo !== undefined && ultimo >= 7 && anteultimo >= 7) return;
      if (ultimo < 5) {
        grupos.riesgo.push({ ...f, puntaje: ultimo });
      } else {
    ...
    // Si no hay nada para repasar
    if (!grupos.riesgo.length && !grupos.mejorar.length && !grupos.sinevaluar.length) {
      wrap.innerHTML = `
        <div style="text-align:center;padding:60px 20px;color:var(--muted2);">
          <div style="font-size:48px;margin-bottom:16px;">🎉</div>
          <h3 style="color:#f0f0f0;font-size:18px;margin-bottom:8px;">¡Estás al día!</h3>
          <p style="font-size:14px;">No hay filminas pendientes de repasar.</p>
        </div>`;
      return;
    }

    // Mensaje motivacional según prioridad
    const totalRepaso = grupos.riesgo.length + grupos.mejorar.length;
    let mensajeTop = '';
    if (grupos.riesgo.length > 0) {
      mensajeTop = `<div onclick="iniciarRepasoEnfocado()" style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.25);border-radius:14px;padding:16px 20px;margin-bottom:20px;display:flex;align-items:center;gap:14px;cursor:pointer;transition:background .15s;" onmouseover="this.style.background='rgba(239,68,68,0.14)'" onmouseout="this.style.background='rgba(239,68,68,0.08)'">
        <div style="font-size:28px;">⚠️</div>
        <div style="flex:1;">
          <div style="font-size:14px;font-weight:700;color:#ef4444;margin-bottom:2px;">Tenés ${grupos.riesgo.length} filmina${grupos.riesgo.length !== 1 ? 's' : ''} en riesgo</div>
          <div style="font-size:13px;color:var(--muted);">Click para repasar las ${totalRepaso} filminas pendientes →</div>
        </div>
      </div>`;
    } else if (grupos.mejorar.length > 0) {
      mensajeTop = `<div onclick="iniciarRepasoEnfocado()" style="background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.25);border-radius:14px;padding:16px 20px;margin-bottom:20px;display:flex;align-items:center;gap:14px;cursor:pointer;transition:background .15s;" onmouseover="this.style.background='rgba(245,158,11,0.14)'" onmouseout="this.style.background='rgba(245,158,11,0.08)'">
        <div style="font-size:28px;">📈</div>
        <div style="flex:1;">
          <div style="font-size:14px;font-weight:700;color:#f59e0b;margin-bottom:2px;">${grupos.mejorar.length} filmina${grupos.mejorar.length !== 1 ? 's' : ''} para repasar</div>
          <div style="font-size:13px;color:var(--muted);">Click para repasarlas →</div>
        </div>
      </div>`;
    }
        ...
        <div style="background:${bgColor};border:1px solid ${color}40;border-radius:14px;padding:18px 20px;margin-bottom:16px;">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;">
            ${lista.slice(0, 15).map(f => `
              <div style="display:flex;justify-content:space-between;align-items:center;background:rgba(0,0,0,0.2);border-radius:8px;padding:10px 12px;cursor:pointer;transition:background .15s;" onmouseover="this.style.background='rgba(0,0,0,0.35)'" onmouseout="this.style.background='rgba(0,0,0,0.2)'" onclick="abrirFilminaDesdeRepaso(${f.id})">
            ...
            `).join('')}
            ${lista.length > 15 ? `<p style="font-size:12px;color:var(--muted2);text-align:center;margin:4px 0 0;">+ ${lista.length - 15} más</p>` : ''}
          </div>
        </div>`;
    };

    // ✨ Calcular "Plan de hoy" si hay fecha de examen
    const planKey = `plan_fecha_examen_${currentUser.id}_${currentMateria || 'global'}`;
    const fechaExamenStr = localStorage.getItem(planKey);
    let planHoyHtml = '';
    ...
    if (fechaExamenStr) {
      ...
      const examen = new Date(fechaExamenStr + 'T00:00:00');
      const diasRestantes = Math.ceil((examen - hoyDate) / (1000 * 60 * 60 * 24));
      const diasEfectivos = Math.max(1, diasRestantes - MARGEN_DIAS);

      // Calcular aprendidas y pendientes
      const aprendidasIds = new Set();
      Object.entries(scoresPorFilmina).forEach(([id, arr]) => {
        if (arr.length >= 2 && arr[0].puntaje >= 7 && arr[1].puntaje >= 7) {
          aprendidasIds.add(parseInt(id));
      ...
      const pendientes = filminas.filter(f => !aprendidasIds.has(parseInt(f.id)));
      const filminasPorDia = Math.ceil(pendientes.length / diasEfectivos);
      ...
      const estudiadasHoyCount = estudiadasHoyIds.size;
      const faltanHoy = Math.max(0, filminasPorDia - estudiadasHoyCount);

      // ✨ Sugeridas: primero por urgencia (riesgo/mejorar/nuevas), después por orden natural (unidad + orden de carga)
      const noEstudiadasHoy = pendientes.filter(f => !estudiadasHoyIds.has(parseInt(f.id)));
      const ordenPrioridad = noEstudiadasHoy.map(f => {
        const fScores = scoresPorFilmina[parseInt(f.id)];
        if (!fScores?.length) return { ...f, prio: 2, puntaje: null };
        const ultimo = fScores[0].puntaje;
        if (ultimo < 5) return { ...f, prio: 0, puntaje: ultimo };
        if (ultimo < 7) return { ...f, prio: 1, puntaje: ultimo };
        return { ...f, prio: 3, puntaje: ultimo };
      });
      ordenPrioridad.sort((a, b) => {
        // 1° prioridad: urgencia
        if (a.prio !== b.prio) return a.prio - b.prio;
        // 2° prioridad: dentro de "en riesgo" y "por mejorar", primero las peores
        if (a.prio <= 1 && a.puntaje !== b.puntaje) return (a.puntaje ?? 99) - (b.puntaje ?? 99);
        // 3° prioridad: por número de unidad (U1 antes que U2)
        const uA = a.unidades?.numero ?? 999;
        const uB = b.unidades?.numero ?? 999;
        if (uA !== uB) return uA - uB;
        // 4° prioridad: por orden de carga dentro de la unidad
        return (a.orden ?? 0) - (b.orden ?? 0);
      });
      const sugeridas = ordenPrioridad.slice(0, faltanHoy);

      const completo = estudiadasHoyCount >= filminasPorDia;
      const pct = Math.min(100, Math.round((estudiadasHoyCount / filminasPorDia) * 100));
      const colorBarra = completo ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#7c6dfa';

      planHoyHtml = `
        <div style="background:rgba(124,109,250,0.06);border:1px solid rgba(124,109,250,0.25);border-radius:14px;padding:18px 20px;margin-bottom:20px;">
          <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px;flex-wrap:wrap;gap:8px;">
            <span style="font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:var(--accent);">📅 Plan de hoy</span>
            <span style="font-size:13px;font-weight:700;color:${colorBarra};">${estudiadasHoyCount}/${filminasPorDia} ${completo ? '✓' : ''}</span>
          </div>
          <div style="background:rgba(255,255,255,0.08);border-radius:99px;height:6px;overflow:hidden;margin-bottom:12px;">
            <div style="height:100%;border-radius:99px;background:${colorBarra};width:${pct}%;transition:width 0.6s ease;"></div>
          </div>
          ${completo
            ? `<p style="font-size:13px;color:#22c55e;margin:0;">🎉 ¡Meta del día cumplida! Igual podés repasar más abajo.</p>`
            : sugeridas.length === 0
              ? `<p style="font-size:13px;color:var(--muted);margin:0;">No quedan filminas sugeridas para hoy.</p>`
              : `
                <p style="font-size:12px;color:var(--muted);margin:0 0 10px;">Te recomendamos estudiar estas ${sugeridas.length} filmina${sugeridas.length !== 1 ? 's' : ''} hoy (prioridad por urgencia):</p>
                <div style="display:flex;flex-direction:column;gap:6px;">
                  ${sugeridas.map(f => {
                    const colorP = f.puntaje === null ? '#94a3b8' : f.puntaje < 5 ? '#ef4444' : f.puntaje < 7 ? '#f59e0b' : '#22c55e';
                    const etiquetaP = f.puntaje === null ? 'Nueva' : `${f.puntaje}/10`;
                    return `
                      <div onclick="abrirFilminaDesdeRepaso(${f.id})" style="display:flex;justify-content:space-between;align-items:center;background:rgba(0,0,0,0.2);border-radius:8px;padding:10px 12px;cursor:pointer;transition:background .15s;" onmouseover="this.style.background='rgba(0,0,0,0.35)'" onmouseout="this.style.background='rgba(0,0,0,0.2)'">
                        <div style="min-width:0;overflow:hidden;flex:1;">
                          <span style="font-size:11px;color:#64748b;">U${f.unidades?.numero || '?'} · </span>
                          <span style="font-size:13px;color:#f1f5f9;">${escHtml(f.titulo)}</span>
                        </div>
                        <span style="font-size:12px;font-weight:700;color:${colorP};flex-shrink:0;margin-left:8px;">${etiquetaP}</span>
                      </div>
                    `;
                  }).join('')}
                </div>
    ...
    console.error('Error renderRepaso:', e);
    wrap.innerHTML = '<p style="color:var(--muted2);padding:20px;">Error al cargar el repaso.</p>';
  }
}

async function abrirFilminaDesdeRepaso(filminaId) {
  try {
    // Obtener la filmina con su unidad para saber a dónde ir
    const { data: filmina } = await supabaseClient
      .from('filminas')
      .select('id, unidad_id, unidades(numero, grupo_id)')
      .eq('id', filminaId)
      .single();

    if (!filmina) return;

    const unidadNumero = filmina.unidades.numero;
    const grupoId = filmina.unidades.grupo_id;

    // Setear flag para que renderFilminas la expanda
    pendingFilminaExpand = parseInt(filminaId);
    currentGrupoId = grupoId;

    await showSection('teoria');
    openUnidad(unidadNumero);
  } catch (e) {
    console.error('Error abriendo filmina:', e);
  }
}

async function iniciarRepasoEnfocado() {
  document.getElementById('view-repaso-list').style.display = 'none';
  document.getElementById('view-repaso-focus').style.display = 'block';
  window.scrollTo(0, 0);

  const wrap = document.getElementById('repaso-focus-wrap');
  wrap.innerHTML = `<div style="text-align:center;padding:40px 0;color:var(--muted)"><div style="font-size:32px;margin-bottom:12px">⏳</div><p>Preparando filminas para repaso...</p></div>`;

  try {
    const visibleIds = await getVisibleUserIds();

    // Traer filminas con contenido
    let filminasQuery = supabaseClient
      .from('filminas')
      .select('id, titulo, contenido, imagen_url, unidades(numero, nombre)')
      .in('alumno_id', visibleIds);
    if (currentMateria) filminasQuery = filminasQuery.eq('grupo_id', currentMateria);
    const { data: todasFilminas } = await filminasQuery;
    ...
    // Filtrar solo las que necesitan repaso
    const paraRepasar = [];
    filminas.forEach(f => {
      const fScores = scoresPorFilmina[parseInt(f.id)];
      if (!fScores?.length) return;
      ...
      const anteultimo = fScores[1]?.puntaje;
      if (anteultimo !== undefined && ultimo >= 7 && anteultimo >= 7) return;
      paraRepasar.push({ ...f, puntaje: ultimo });
    });

    // Ordenar: primero las de menor puntaje
    paraRepasar.sort((a, b) => a.puntaje - b.puntaje);

    if (!paraRepasar.length) {
      wrap.innerHTML = `<div style="text-align:center;padding:40px;color:var(--muted2);"><p>No hay filminas para repasar.</p></div>`;
      return;
    }
    ...
    wrap.innerHTML = `
      <div class="section-hero">
        <div class="sh-badge">📖 Sesión de repaso</div>
        <h2>Repasando ${paraRepasar.length} filmina${paraRepasar.length !== 1 ? 's' : ''}</h2>
        <p>Leé con atención. Las que tenés peor puntaje aparecen primero.</p>
      </div>

      <div style="display:flex;flex-direction:column;gap:16px;margin-top:20px;">
        ${paraRepasar.map((f, idx) => {
          const contenidoHtml = /<[a-z][\s\S]*>/i.test(f.contenido)
            ? f.contenido
            : (f.contenido || '').replace(/\n/g, '<br>');
          return `
            <div style="background:#2a2a2a;border:1px solid var(--border);border-radius:14px;padding:20px 22px;">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;gap:12px;flex-wrap:wrap;">
                <div style="display:flex;align-items:center;gap:10px;min-width:0;flex:1;">
                  <span style="background:rgba(255,255,255,0.06);border:1px solid var(--border);border-radius:8px;padding:4px 10px;font-size:11px;color:var(--muted);font-weight:600;">${idx + 1}/${paraRepasar.length}</span>
                  <span style="font-size:11px;color:#64748b;">U${f.unidades?.numero || '?'} · ${escHtml(f.unidades?.nombre || '')}</span>
                </div>
                <span style="font-size:13px;font-weight:700;color:${getColor(f.puntaje)};">Último: ${f.puntaje}/10</span>
              </div>
              <h3 style="color:#f1f5f9;font-size:17px;margin-bottom:14px;font-weight:700;">${escHtml(f.titulo)}</h3>
              ${f.imagen_url ? `<img src="${f.imagen_url}" style="max-width:100%;border-radius:8px;margin-bottom:14px" />` : ''}
              <div style="color:#cbd5e1;font-size:14px;line-height:1.7;">${contenidoHtml}</div>
            </div>`;
        }).join('')}
      </div>

      <div style="margin-top:24px;padding:18px;background:rgba(56,189,248,0.06);border:1px solid rgba(56,189,248,0.2);border-radius:12px;text-align:center;">
        <p style="font-size:13px;color:var(--muted);margin-bottom:10px;">¿Ya repasaste todo?</p>
        <p style="font-size:14px;color:#f0f0f0;font-weight:600;">💡 Hacé un Pomodoro para evaluar cuánto retuviste y subir tu puntaje.</p>
      </div>
    `;
  } catch (e) {
    console.error('Error iniciarRepasoEnfocado:', e);
    wrap.innerHTML = '<p style="color:var(--muted2);padding:20px;">Error al cargar las filminas.</p>';
  }
}

function iniciarPomodoroDesdeRepaso() {
  if (pomodoroActivo) return;
  // Las filminas ya están seteadas en window.filminasActuales por iniciarRepasoEnfocado
  // Solo abrimos el modal de configuración del Pomodoro
  abrirConfigPomodoro();
}

function volverRepasoList() {
  if (pomodoroActivo) {
    if (!confirm('Tenés un Pomodoro activo. Si volvés se va a perder. ¿Continuar?')) return;
    finalizarPomodoro();
  }
  document.getElementById('view-repaso-focus').style.display = 'none';
  document.getElementById('view-repaso-list').style.display = 'block';
  window.scrollTo(0, 0);
  renderRepaso();
}
...
async function iniciarRepasoEnfocado() {
  ...
  const wrap = document.getElementById('repaso-focus-wrap');
  wrap.innerHTML = `<div style="text-align:center;padding:40px 0;color:var(--muted)"><div style="font-size:32px;margin-bottom:12px">⏳</div><p>Preparando filminas...</p></div>`;
  ...
  try {
    ...
    // ✨ Setear como filminas activas para que el Pomodoro las use
    window.filminasActuales = paraRepasar;
    ...
    wrap.innerHTML = `
      <div class="section-hero">
        <div class="sh-badge">📖 Sesión de repaso</div>
        <h2>Repasando ${paraRepasar.length} filmina${paraRepasar.length !== 1 ? 's' : ''}</h2>
        <p>Leé con atención y expandí cada filmina. Al terminar, la IA va a evaluar cuánto recordás.</p>
      </div>

      <!-- Barra Pomodoro -->
      <div style="display:flex; align-items:center; gap:12px; margin:20px 0; flex-wrap:wrap;">
        <button class="dice-btn" onclick="iniciarPomodoroDesdeRepaso()" id="pomodoro-btn-repaso" title="Pomodoro de repaso">
          <span class="dice-icon">🍅</span>
          <span class="dice-label" style="color:#f0f0f0;">Iniciar Pomodoro de Repaso</span>
        </button>
        <div id="pomodoro-ring-repaso" style="display:none; align-items:center; gap:12px; background:rgba(212,175,55,0.08); border:1px solid rgba(212,175,55,0.2); border-radius:12px; padding:8px 16px;">
          <svg width="52" height="52" viewBox="0 0 52 52">
            <circle cx="26" cy="26" r="21" fill="none" stroke="rgba(212,175,55,0.15)" stroke-width="4"/>
            <circle cx="26" cy="26" r="21" fill="none" stroke="#d4af37" stroke-width="4"
              stroke-dasharray="131.9" stroke-dashoffset="0" stroke-linecap="round"
              transform="rotate(-90 26 26)" class="ring-progress"/>
            <text x="26" y="30" text-anchor="middle" fill="#f0e8d0" font-size="10" font-weight="700" class="ring-time">25:00</text>
          </svg>
          <div style="display:flex;flex-direction:column;gap:4px;">
            <div style="font-size:10px;letter-spacing:0.12em;text-transform:uppercase;color:rgba(212,175,55,0.7);">Sesión activa</div>
            <div style="display:flex;gap:6px;">
              <button onclick="pausarPomodoro()" style="padding:4px 12px;border-radius:6px;background:rgba(212,175,55,0.15);border:none;color:#d4af37;font-size:11px;font-weight:600;cursor:pointer;">Pausar</button>
              <button onclick="finalizarPomodoro()" style="padding:4px 12px;border-radius:6px;background:rgba(34,197,94,0.15);border:none;color:#4ade80;font-size:11px;font-weight:600;cursor:pointer;">Finalizar</button>
            </div>
          </div>
        </div>
      </div>

      <!-- Filminas con estructura de cards (compatible con Pomodoro) -->
      <div class="cards-container">
        ${paraRepasar.map((f, idx) => {
          const contenidoHtml = /<[a-z][\s\S]*>/i.test(f.contenido)
            ...
            : (f.contenido || '').replace(/\n/g, '<br>');
          return `
            <div class="card ${colores[idx % colores.length]}" id="card-${f.id}" data-filmina-id="${f.id}">
              <div class="card-header" onclick="toggleCard('${f.id}'); registrarFilminaLeida('${f.id}')">
                <div class="card-num">${idx + 1}</div>
                <div class="card-info">
                  <div class="card-title">${escHtml(f.titulo)}</div>
                  <div style="font-size:11px;color:var(--muted2);margin-top:3px;">U${f.unidades?.numero || '?'} · ${escHtml(f.unidades?.nombre || '')} · <span style="color:${getColor(f.puntaje)};font-weight:700;">Último: ${f.puntaje}/10</span></div>
                </div>
                <button class="card-toggle" aria-label="expandir">▼</button>
              </div>
              <div class="card-body">
                <div class="card-body-inner">
                  ${f.imagen_url ? `<img src="${f.imagen_url}" style="max-width:100%;border-radius:8px;margin-bottom:12px" />` : ''}
                  <div class="filmina-content">${contenidoHtml}</div>
                </div>
              </div>
            </div>`;
        }).join('')}
    ...
    // Re-inicializar lucide para los íconos nuevos
...
async function renderWidgetPlanHoy(reintentos = 0) {
  const widget = document.getElementById('widget-plan-hoy');
  if (!widget) return;

  // ✨ Si no hay user todavía, reintentar hasta 5 veces (cada 500ms)
  if (!currentUser) {
    if (reintentos < 5) {
      setTimeout(() => renderWidgetPlanHoy(reintentos + 1), 500);
    }
    return;
  }

  try {
    // ✨ Recuperar materia desde localStorage si currentMateria todavía está null
    const materiaParaKey = currentMateria || localStorage.getItem('materia_' + currentUser.id) || 'global';
    const planKey = `plan_fecha_examen_${currentUser.id}_${materiaParaKey}`;
    const fechaExamenStr = localStorage.getItem(planKey);

    if (!fechaExamenStr) {
      // Probar con 'global' por si guardó sin materia
      const globalKey = `plan_fecha_examen_${currentUser.id}_global`;
      const fechaGlobal = localStorage.getItem(globalKey);
      if (!fechaGlobal) {
    ...
    const fechaFinal = fechaExamenStr || localStorage.getItem(`plan_fecha_examen_${currentUser.id}_global`);
    ...
    const MARGEN_DIAS = 3;
    const hoy = new Date(); hoy.setHours(0, 0, 0, 0);
    const examen = new Date(fechaFinal + 'T00:00:00');
    const diasRestantes = Math.ceil((examen - hoy) / (1000 * 60 * 60 * 24));
    const diasEfectivos = Math.max(1, diasRestantes - MARGEN_DIAS);
    const cantPend = filminas.filter(f => !aprendidasIds.has(parseInt(f.id))).length;
    const filminasPorDia = Math.ceil(cantPend / diasEfectivos);

    if (filminasPorDia === 0 || filminas.length === 0) {
      widget.style.display = 'none';
      return;
    }

    const hoyStr = hoy.toISOString().split('T')[0];
    const estudiadasHoy = new Set(
      scores
        .filter(s => (s.created_at || '').startsWith(hoyStr) && s.puntaje >= 5)
        .map(s => parseInt(s.filmina_id))
    ).size;

    const pct = Math.min(100, Math.round((estudiadasHoy / filminasPorDia) * 100));
    const completo = estudiadasHoy >= filminasPorDia;
    const colorBarra = completo ? '#22c55e' : pct >= 50 ? '#f59e0b' : '#7c6dfa';

    widget.style.display = 'block';
    widget.innerHTML = `
      <div onclick="showSection('repaso')" style="background:rgba(124,109,250,0.08);border:1px solid rgba(124,109,250,0.25);border-radius:14px;padding:16px 20px;margin-bottom:16px;cursor:pointer;transition:background .15s;" onmouseover="this.style.background='rgba(124,109,250,0.14)'" onmouseout="this.style.background='rgba(124,109,250,0.08)'">
        <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px;flex-wrap:wrap;gap:8px;">
          <div>
            <span style="font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:var(--accent);">🎯 Plan de hoy</span>
            <div style="font-size:15px;color:#f0f0f0;font-weight:600;margin-top:4px;">
              ${completo ? '✓ ¡Meta cumplida!' : `${estudiadasHoy} de ${filminasPorDia} filminas estudiadas`}
            </div>
          </div>
          <span style="font-size:22px;font-weight:700;color:${colorBarra};">${pct}%</span>
        </div>
        <div style="background:rgba(255,255,255,0.08);border-radius:99px;height:8px;overflow:hidden;">
          <div style="height:100%;border-radius:99px;background:${colorBarra};width:${pct}%;transition:width 0.6s ease;"></div>
        </div>
        <div style="font-size:11px;color:var(--muted2);margin-top:8px;">
          ${completo
            ? '🎉 Llevás un excelente ritmo. Igual podés repasar más si querés.'
            : `Click acá para ver qué filminas estudiar →`}
        </div>
      </div>
    `;
  } catch (e) {
    console.error('Error renderWidgetPlanHoy:', e);
    widget.style.display = 'none';
  }
}
