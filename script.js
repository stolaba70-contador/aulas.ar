...
async function mostrarResumenProgreso() {
  ...
  try {
    ...
    // 2. Sesiones Pomodoro
    let pomodorosQuery = supabaseClient
  .from('sesiones_pomodoro')
  .select('duracion_segundos')
  .eq('alumno_id', currentUser.id);
if (currentMateria) pomodorosQuery = pomodorosQuery.eq('grupo_id', currentMateria);
const { data: pomodoros } = await pomodorosQuery;
...
const { data: filminasResumen } = await supabaseClient
  ...
  .in('alumno_id', visibleIdsResumen);
    ...
    // Días sin estudiar
    const fechasSesiones = (sesiones || [])
      .map(s => s.fecha?.split('T')[0])
      .filter(Boolean)
      .sort()
      .reverse();
    const ultimaSesion = fechasSesiones[0] || null;
...
(sesiones || []).forEach(s => {
  ...
  if (total > 0) sesionsPorUnidad[u].push(total);
    ...
    document.getElementById('resumen-body').innerHTML = `
      ...
      ${proximoExamen ? `
      <div class="resumen-alerta examen" style="background:rgba(220,80,60,0.15);border-color:rgba(220,80,60,0.4);margin-bottom:12px">
        <div class="ra-icon">📅</div>
        <div>
          <div class="ra-titulo">Próximo examen: ${escHtml(proximoExamen.titulo)}</div>
          <div class="ra-subtitulo">${diasParaExamen === 0 ? '¡Es hoy!' : diasParaExamen === 1 ? 'Mañana' : `En ${diasParaExamen} días`}</div>
        </div>
        <button class="ra-btn" onclick="cerrarResumen(); showSection('autoevaluacion')">Practicar →</button>
      </div>` : ''}

      ${diasSinEstudiar !== null && diasSinEstudiar >= 3 ? `
      <div class="resumen-alerta debil" style="margin-bottom:12px">
        <div class="ra-icon">😴</div>
        <div>
          <div class="ra-titulo">Llevas ${diasSinEstudiar} días sin estudiar</div>
          <div class="ra-subtitulo">¡Retomá el ritmo hoy!</div>
        </div>
    ...
    <div class="resumen-chip">
      <div class="rc-valor">${totalSesiones}</div>
      <div class="rc-label">Tests completados</div>
    </div>
    <div class="resumen-chip">
      <div class="rc-valor">${minutosEstudio}</div>
      <div class="rc-label">Min. estudiados</div>
    </div>
    ${racha > 1 ? `
    <div class="resumen-chip" style="border-color:rgba(255,160,0,0.4)">
      <div class="rc-valor" style="color:#FFA000">🔥 ${racha}</div>
      <div class="rc-label">Días seguidos</div>
    </div>` : ''}
    ${promedioMC !== null ? `
    <div class="resumen-chip" style="border-color:rgba(80,200,120,0.4)">
      <div class="rc-valor" style="color:${promedioMC >= 70 ? '#50C878' : '#FF6B6B'}">${promedioMC}%</div>
      <div class="rc-label">Aciertos múltiple</div>
    </div>` : ''}
    ${fcDominadas > 0 ? `
    <div class="resumen-chip">
      <div class="rc-valor">${fcDominadas}</div>
      <div class="rc-label">Flashcards dominadas</div>
    </div>` : ''}
    <div class="resumen-chip" style="${fcParaRepasar > 0 ? 'border-color:rgba(255,100,100,0.4)' : ''}">
      ...
      ${puntoDebil ? `
      <div class="resumen-alerta debil">
        <div class="ra-icon">⚠️</div>
        <div>
          <div class="ra-titulo">Menos practicada</div>
          <div class="ra-subtitulo">${escHtml(puntoDebil[0])} — ${puntoDebil[1]} test${puntoDebil[1] !== 1 ? 's' : ''}</div>
        </div>
        <button class="ra-btn" onclick="cerrarResumen(); showSection('teoria')">Repasar →</button>
      </div>` : ''}

      ${puntoFuerte && puntoFuerte[0] !== puntoDebil?.[0] ? `
      <div class="resumen-alerta fuerte">
        <div class="ra-icon">💪</div>
        <div>
          <div class="ra-titulo">Más practicada</div>
          <div class="ra-subtitulo">${escHtml(puntoFuerte[0])} — ${puntoFuerte[1]} test${puntoFuerte[1] !== 1 ? 's' : ''}</div>
        </div>
      </div>` : ''}

      ${fcParaRepasar > 0 ? `
      <div class="resumen-alerta neutral">
        <div class="ra-icon">🃏</div>
        <div>
          <div class="ra-titulo">Flashcards pendientes</div>
          <div class="ra-subtitulo">${fcParaRepasar} tarjeta${fcParaRepasar !== 1 ? 's' : ''} para repasar hoy</div>
        </div>
        <button class="ra-btn" onclick="cerrarResumen(); showSection('teoria')">Ver →</button>
      </div>` : ''}
      ${(todasUnidades || []).length > 0 ? `
<div style="margin-top:16px">
  <div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:10px">Progreso por unidad</div>
  ${(todasUnidades || []).map(u => {
    const filminasDeUnidad = filminasPorUnidad[u.id] || [];
    const totalFilminas = filminasDeUnidad.length;
    const filminasRepasadas = filminasDeUnidad.filter(id => repasosPorFilmina[id]).length;
    ...
    const estado = progreso >= 71 ? '✅ Dominado' : progreso >= 41 ? '📈 En progreso' : '🔴 Necesita repasar';
    return `
    <div style="background:rgba(255,255,255,0.03);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:8px">
