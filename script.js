...
function showAuthScreen(screen) {
  ...
  if (screen === 'login') document.getElementById('auth-login').style.display = 'flex';
  else if (screen === 'register') document.getElementById('auth-register').style.display = 'flex';
  else if (screen === 'pending') document.getElementById('auth-pending').style.display = 'flex';
}
...
async function cargarBannerExamen() {
  ...
  banner.innerHTML = data.map(ev => {
    ...
    return dias === 0
      ...
      : `<div>📅 <strong>Faltan ${dias} días</strong> para <em>${ev.titulo}</em> · ¿Hoy repasaste?</div>`;
  }).join('');
}
...
if (mascota) mascota.style.display = (s === 'welcome' && personajeActual !== 'ninguno') ? 'block' : 'none';
  ...
  const { count } = await supabaseClient
    ...
    .in('alumno_id', visibleIds);
  document.getElementById('count-teoria').textContent = count || 0;
}
else if (s === 'preguntas') {
    ...
    const { count } = await supabaseClient
      ...
      .select('id', { count: 'exact', head: true })
      .in('alumno_id', visibleIds);
    document.getElementById('count-preguntas').textContent = count || 0;
  }
...
async function renderHistorial() {
  ...
  container.innerHTML = historial.map((sesion, si) => `
...
async function renderProgreso() {
    ...
    <button onclick="document.getElementById('pomodoro-filtro-fecha').value=''; filtrarSesionesPomodoro()" style="background:transparent; border:none; color:var(--muted); font-size:12px; cursor:pointer; padding:4px 8px;">✕ Limpiar</button>
      ...
      <div style="display:flex; gap:16px; margin-bottom:16px; flex-wrap:wrap;">
        <div style="background:rgba(124,109,250,0.08); border:1px solid rgba(124,109,250,0.2); border-radius:10px; padding:12px 20px; text-align:center;">
          <div style="font-size:24px; font-weight:700; color:var(--accent)">${sp.length}</div>
        ...
        <div style="background:rgba(56,189,248,0.08); border:1px solid rgba(56,189,248,0.2); border-radius:10px; padding:12px 20px; text-align:center;">
          <div style="font-size:24px; font-weight:700; color:var(--accent2)">${promPuntaje || '-'}/10</div>
        ...
        <div style="background:rgba(224,184,74,0.08); border:1px solid rgba(224,184,74,0.2); border-radius:10px; padding:12px 20px; text-align:center;">
          <div style="font-size:24px; font-weight:700; color:var(--gold)">${sp[sp.length-1]?.nivel_sesion || sp.length}</div>
          <div style="font-size:11px; color:var(--muted); text-transform:uppercase; letter-spacing:0.08em;">Nivel actual</div>
        </div>
      </div>
      <div id="pomodoro-sesiones-lista" style="display:flex; flex-direction:column; gap:10px;" data-sesiones='${JSON.stringify(sp.map(s => ({...s, resumen: s.resumen?.substring(0,200), feedback_ia: s.feedback_ia?.substring(0,300)})))}'>
  ${ultimasSesiones.map(s => {
          ...
          <div style="background:rgba(255,255,255,0.03); border:1px solid var(--border); border-radius:10px; padding:14px 16px;">
            ...
            <p style="font-size:13px; color:var(--muted);">${s.feedback_ia?.substring(0, 180)}${s.feedback_ia?.length > 180 ? '...' : ''}</p>
          </div>`;
        }).join('')}
      </div>
    </div>`;
...
function iniciarPomodoro() {
  ...
  btn.innerHTML = '<span class="dice-icon">⏸</span><span class="dice-label">Pausar</span>';

  const finBtn = document.getElementById('pomodoro-fin-btn');
  ...
  document.getElementById('pomodoro-timer').style.display = 'inline-flex';
  document.getElementById('pomodoro-timer-text').textContent = `${m}:00`;
...
function pausarPomodoro() {
  ...
  if (pomodoroIntervalo) {
    ...
    btn.innerHTML = '<span class="dice-icon">▶️</span><span class="dice-label">Reanudar</span>';
...
function finalizarPomodoro() {
  ...
  const btn = document.getElementById('pomodoro-btn');
  btn.onclick = iniciarPomodoro;
  btn.innerHTML = '<span class="dice-icon">🍅</span><span class="dice-label">Pomodoro</span>';
...
Sé breve y directo. Respondé en español argentino.`;
 ...
 try {
    ...
    const puntajeMatch = feedback.match(/[Pp]untaje[:\s]+([1-9]|10)/);
