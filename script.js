...
if (s === 'welcome') {
  setTimeout(() => verificarRepasosHoy(), 500);
  setTimeout(() => renderWidgetPlanHoy(), 500);
}
...
async function renderWidgetPlanHoy() {
  const widget = document.getElementById('widget-plan-hoy');
  if (!widget || !currentUser) return;

  try {
    // Solo tiene sentido si hay fecha de examen
    const planKey = `plan_fecha_examen_${currentUser.id}_${currentMateria || 'global'}`;
    const fechaExamenStr = localStorage.getItem(planKey);
    if (!fechaExamenStr) { widget.style.display = 'none'; return; }
    ...
    // ✨ Estudiadas hoy = scores de hoy con puntaje ≥ 5 (únicas por filmina)
